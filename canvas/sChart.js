function sBarChart(canvas, data, options) {
    ...
    this.fillColor = 'pink'; // 默认填充颜色
    this.axisColor = '#666666'; // 坐标轴颜色
    this.contentColor = '#eeeeee'; // 内容横线颜色
    this.titleColor = 'blue'; // 图表标题颜色
    this.title = ''; // 图表标题
    this.titlePosition = 'bottom'; // 图表标题位置: top / bottom
...
sBarChart.prototype = {
    init: function(options) {
        if (options) {
            ...
            this.fillColor = options.fillColor || 'pink';
            this.axisColor = options.axisColor || '#666666';
            this.contentColor = options.contentColor || '#eeeeee';
            this.titleColor = options.titleColor || 'blue';
            this.title = options.title;
            this.titlePosition = options.titlePosition || 'bottom';
        }
    ...
    drawPoint: function() {
        ...
        for (var i = 0; i < this.yEqual; i++) {
            // var y = this.yFictitious * (i + 0.5);
            // var ylen = this.yLength * (i + 0.5);
            var y = this.yFictitious * (i / 2 + 0.5);
            var ylen = this.yLength * (i + 0.5);
            this.ctx.beginPath();
            this.ctx.strokeStyle = this.axisColor;
